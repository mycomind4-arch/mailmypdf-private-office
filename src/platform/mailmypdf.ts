...
export interface PrivateOfficeRecipient {
  name: string;
...
export interface CreateCommunicationInput {
  document_id: string;
  recipient: PrivateOfficeRecipient;
  mail_type: MailType;
...
export async function uploadDocument(
  ...
  const result = await request<
    { document?: MailMyPDFDocument } | MailMyPDFDocument
  >("/api/v1/documents", { method: "POST", body: form });
  return "document" in result && result.document
    ? result.document
    : (result as MailMyPDFDocument);
}
...
export async function getCommunication(
  ...
  return request<MailMyPDFCommunication>(
    `/api/v1/communications/${encodeURIComponent(id)}`,
  );
}
