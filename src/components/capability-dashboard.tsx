 ...
 * life state, recommended next actions, and a goal planner.
 */
...
interface CapabilityDashboardProps {
  /** Optional persisted user state — if not provided, uses empty initial state */
...
export function CapabilityDashboard({ initialState }: CapabilityDashboardProps) {
  ...
  function handleComplete(capabilityId: string) {
    const result = completeCapability(capabilityGraph, state, capabilityId);
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <section className="mb-8">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatTile
            ...
            icon={<Circle size={16} className="text-brass" />}
          />
          <StatTile
            label="Milestones"
            value={summary.reachedMilestones.length}
            icon={<TrendingUp size={16} className="text-navy" />}
          />
          <StatTile
            label="Available Now"
            value={summary.availableCapabilities.length}
            icon={<ArrowRight size={16} className="text-brass" />}
          />
        </div>
        ...
        {summary.reachedMilestones.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-2">
            {summary.reachedMilestones.map((ms) => (
              <span
                key={ms.id}
                className="rounded-full border border-brass/30 bg-brass/5 px-3 py-1 text-xs font-medium text-brass"
              >
                ✓ {ms.title}
              </span>
            ))}
      ...
      <section className="mb-8 rounded-xl border border-rule bg-paper p-6">
        ...
        <p className="mt-1 text-sm text-stone">
          Tell us your goal and we'll map the path from where you are now to where you want to be.
        </p>
        ...
        {goalText && !goalResult && (
          <p className="mt-3 text-xs text-stone">
            No matching capability found. Try different keywords.
          </p>
        )}
                  ...
                  <li
                    ...
                    className={`flex items-start gap-3 rounded-lg border p-3 ${
                      isAvailable
                        ...
                        : "border-rule bg-ivory"
                    }`}
                  >
                    <span className="mt-0.5 font-mono text-xs text-stone">
                      {idx + 1}
                    </span>
                    <div className="flex-1">
                      ...
                      {isAvailable && (
                        <div className="mt-1 text-xs font-medium text-brass">
                          Available now
                        </div>
                      )}
                      {isBlocked && (
                        <div className="mt-1 flex items-center gap-1 text-xs text-stone">
                          <Lock size={11} /> Blocked by: {isBlocked.join(", ")}
                        </div>
                      )}
                    </div>
                  </li>
      ...
      <section className="mb-8">
        ...
        {recommendations.length === 0 && (
          <div className="mt-4 flex items-center gap-2 rounded-lg border border-rule bg-paper p-4 text-sm text-stone">
            <CheckCircle2 size={16} className="text-emerald-600" />
            You've completed all available capabilities. New workflows may be added as the ecosystem grows.
          </div>
        )}
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {recommendations.map((rec) => (
            <div
              ...
              className="group flex flex-col rounded-xl border border-rule bg-paper p-5 transition-all duration-200 hover:border-navy/30 hover:shadow-premium"
            >
              <div className="font-mono text-[10px] uppercase tracking-widest text-brass">
                {rec.capability.family}
              </div>
              <h4 className="mt-2 text-base text-charcoal">{rec.capability.title}</h4>
              <p className="mt-1 flex-1 text-xs leading-relaxed text-stone">
                {rec.capability.description}
              </p>
              <div className="mt-3 rounded-md bg-ivory px-3 py-2 text-xs text-stone">
                {rec.reason}
              </div>
              <div className="mt-4 flex items-center gap-2">
                {rec.capability.workflowId ? (
                  <Link
                    to={`/workflows/${rec.capability.workflowId}`}
                  ...
                  <button
                    onClick={() => handleComplete(rec.capability.id)}
                    className="text-sm font-medium text-navy transition-colors group-hover:text-brass"
                  >
                    Mark complete <ArrowRight size={14} className="inline" />
                  </button>
                )}
                {rec.capability.workflowId && (
                  <button
                    onClick={() => handleComplete(rec.capability.id)}
                    className="ml-auto text-xs text-stone-light hover:text-stone"
                  >
                    Skip →
                  </button>
                )}
      ...
      {summary.lockedCapabilities.length > 0 && (
        <section className="mb-8">
          ...
          <p className="mt-1 text-sm text-stone-light">
            These become available after you complete prerequisite steps.
          </p>
          <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {summary.lockedCapabilities.map((cap) => (
              <div
                key={cap.id}
                className="flex items-start gap-2 rounded-lg border border-rule bg-ivory/50 p-3"
              >
                <Lock size={12} className="mt-0.5 text-stone-light" />
                <div>
                  <div className="text-xs font-medium text-stone">{cap.title}</div>
                  <div className="mt-0.5 text-[10px] text-stone-light">
                    Requires: {cap.prerequisites.join(", ")}
                  </div>
                </div>
              </div>
            ))}
