...
-- ─────────────────────────────────────────────────────────────────────────
-- LLM Provenance table
--
-- Records every consequential LLM operation for audit and reproducibility.
-- Does NOT store raw prompts or sensitive document content — only hashes
-- and structured metadata.
-- ─────────────────────────────────────────────────────────────────────────

create table if not exists public.private_office_llm_provenance (
  id uuid primary key default gen_random_uuid(),
  matter_id uuid references public.private_office_matters(id) on delete set null,
  owner_id text not null,
  operation text not null check (operation in ('classify','extract','analyze','assess_risk','generate_strategy','assist_draft','reconcile','extract_timeline')),
  provider text not null check (provider in ('gemini','openai','anthropic')),
  model text not null,
  prompt_version text not null,
  input_hash text not null,
  output_hash text not null,
  status text not null check (status in ('success','failed','rejected','fallback_used')),
  fallback_used boolean not null default false,
  fallback_chain jsonb not null default '[]'::jsonb,
  providers_consulted jsonb not null default '[]'::jsonb,
  duration_ms integer,
  workflow_id text,
  temperature numeric,
  max_tokens integer,
...
create index if not exists private_office_llm_provenance_matter_idx on public.private_office_llm_provenance(matter_id, created_at desc);
create index if not exists private_office_llm_provenance_owner_idx on public.private_office_llm_provenance(owner_id, created_at desc);
create index if not exists private_office_llm_provenance_operation_idx on public.private_office_llm_provenance(operation, provider, created_at desc);

-- RLS: client can read only; writes go through server functions (service role)
alter table public.private_office_llm_provenance enable row level security;

drop policy if exists private_office_llm_provenance_select_own on public.private_office_llm_provenance;
create policy private_office_llm_provenance_select_own on public.private_office_llm_provenance for select using (auth.uid()::text = owner_id);
drop policy if exists private_office_llm_provenance_insert_own on public.private_office_llm_provenance;
drop policy if exists private_office_llm_provenance_update_own on public.private_office_llm_provenance;
drop policy if exists private_office_llm_provenance_delete_own on public.private_office_llm_provenance;

-- Updated_at trigger
drop trigger if exists private_office_llm_provenance_updated_at on public.private_office_llm_provenance;
create trigger private_office_llm_provenance_updated_at before update on public.private_office_llm_provenance for each row execute function public.set_private_office_updated_at();
